...
export function DeliveriesCard({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    setData(null);
    setError(false);
    api.deliveries().then(setData).catch(() => setError(true));
  }, [refreshKey]);
    ...
    <div className="card">
      <div className="card-header">
        <div className="card-identity">
          ...
          <div>
            ...
            <div className="card-subtitle">Expected &amp; in transit</div>
          </div>
        </div>
        {data && (
      ...
      {error ? (
        ...
        <div className="delivery-list">
          {all.length === 0 && (
            <div className="empty-state">No deliveries on record.</div>
          )}
          {all.map((d) => (
            <div key={d.id} className="delivery-row">
              <div className="delivery-body">
                ...
                <div className="delivery-meta">
                  {d.id.toUpperCase()} · {d.expectedTimeWindow}
                </div>
              </div>
