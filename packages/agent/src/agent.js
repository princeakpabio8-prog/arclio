...
export async function runAgent(userInput) {
    ...
    try {
        ...
        if (err instanceof PlanValidationError) {
            console.warn(`[agent] Plan validation rejected output from '${provider.name}':`, err.violations.join("; "));
        }
        else {
    ...
    console.log(`[agent] provider: ${provider.name} | intent: ${plan.intent} | steps: ${plan.steps.length}`);
    // Stage 2 — Execute
    const results = await executePlan(plan);
    console.log(`[agent] executed ${results.length} tool(s), ` +
