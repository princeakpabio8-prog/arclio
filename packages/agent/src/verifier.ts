...
export function verifyResults(
  ...
  if (plan.intent === "unknown") {
    return {
      ...
      findings: [
        {
          tool: "get_today_calendar" as ToolName,
          ok: false,
...
function successDetail(result: ToolResult): string {
  const data = result.parsed as Record<string, unknown> | null;
  ...
  switch (result.tool) {
    ...
    case "get_pending_deliveries": {
      const deliveries = data["deliveries"] as unknown[] | undefined;
    ...
    case "notify_procurement": {
      if (data["success"] === true) {
        const n = data["notification"] as Record<string, unknown> | undefined;
